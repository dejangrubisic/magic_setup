...
- keep: extractor fixes (#05) and the report `dev/practice/livebench/REPORT.md`. Drop: nothing. Practice-run notes: #02 and #05 were squash-merged before their 2026-09-03 re-reviews returned (time budget); fresh reviews returned REQUEST_CHANGES on both (02: `item_table`/`binarise` unused by the script, `NaN` in summary.json; 05: strict spatial rule deviates from criterion 1 without an amendment, lenient bold-fallback change is out of scope). OPEN: fix in follow-up issues before this branch is treated as reviewed. Verdict JSON copies: `dev/practice/livebench/reviews/`.
